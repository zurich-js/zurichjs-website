...
export default function FeedbackAnalytics({
  ...
  const [activeTab, setActiveTab] = useState<
    "overview" | "events" | "speakers" | "talks" | "allFeedback"
  >("overview");
  ...
  const exportToCSV = () => {
    ...
    link.setAttribute(
      "download",
      `zurichjs-feedback-export-${new Date().toISOString().split("T")[0]}.csv`,
    );
  ...
  return (
    <Layout>
      <SEO
        title="Feedback Analytics | ZurichJS Admin"
        description={"Analyze all feedback collected from ZurichJS events"}
      />
        ...
        <div className="border-b border-gray-200 mb-6 overflow-x-auto">
          <div className="flex -mb-px min-w-max">
            <button
              ...
              className={`inline-flex items-center px-3 py-2 font-medium text-xs sm:text-sm border-b-2 ${
                activeTab === "overview"
                ...
                <div className="space-y-3 sm:space-y-4">
                  {talkStats
                    ...
                    .map((talk) => (
                      <div
                        key={talk._id}
                        className="flex items-center gap-2 sm:gap-4 p-2 sm:p-3 bg-gray-50 rounded-lg text-sm sm:text-base"
                      >
                    ...
                    .map((speaker) => (
                      ...
                      >
                        ...
                        <div className="flex-grow min-w-0">
                          ...
                          <div className="text-xs sm:text-sm text-gray-500 truncate">
                            {speaker.feedbackCount} feedback
                          </div>
                        </div>
                        <div className="flex items-center whitespace-nowrap bg-yellow-100 text-yellow-800 px-2 py-1 rounded-full text-xs sm:text-sm font-medium">
                  ...
                  {filteredEvents.length === 0 ? (
                    ...
                    filteredEvents.map((event) => (
                      <tr key={event._id} className="hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">{event.feedbackCount}</td>
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="flex items-center">
                            {event.averageRating.toFixed(1)}
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">{event.talkCount}</td>
                      </tr>
                    ))
                  )}
                </tbody>
                  ...
                  {filteredSpeakers.length === 0 ? (
                    ...
                    filteredSpeakers.map((speaker) => (
                      <tr key={speaker._id} className="hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">{speaker.feedbackCount}</td>
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="flex items-center">
                            {speaker.averageRating.toFixed(1)}
                            <Star className="ml-1 h-4 w-4 fill-yellow-400 text-yellow-400" />
