    ...
    <Layout>
      <SEO
        ...
        openGraph={{
          ...
          description:
            "Meet the amazing speakers who have shared their JavaScript knowledge at ZurichJS meetups.",
      ...
      <Section variant="gradient" className="mt-20">
        ...
        >
          <h1 className="text-4xl md:text-5xl font-bold mb-4">Our Amazing Speakers</h1>
          <p className="text-xl max-w-2xl mx-auto">
            Meet the talented developers who have shared their JavaScript expertise at ZurichJS
            meetups!
          </p>
        </motion.div>
              ...
              <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6">
                <div>
                  ...
                  <p className="text-gray-600 text-sm sm:text-base">
                    Our growing JavaScript community in Zurich
                  </p>
                </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4 mb-6">
                ...
                <div className="bg-gradient-to-br from-yellow-50 to-yellow-100 rounded-lg p-3 sm:p-5 flex flex-col shadow-sm">
                  <div className="flex items-center mb-2 sm:mb-3">
                    ...
                    <h3 className="text-sm sm:text-lg font-bold truncate max-w-[110px] sm:max-w-full">
                      {speakerStats?.mostActiveSpeaker?.name || "N/A"}
                    </h3>
                  </div>
                  <p className="text-gray-700 text-xs sm:text-sm font-medium mt-auto">
              ...
              <div className="bg-gradient-to-br from-pink-50 to-pink-100 rounded-lg p-3 sm:p-5 flex flex-col shadow-sm mb-6">
                <div className="flex justify-between items-center mb-3 sm:mb-4">
                  ...
                  <Button
                    ...
                    variant="ghost"
                    size="sm"
                    className="text-pink-600 hover:text-pink-700 text-xs sm:text-sm"
                  >
                ...
                <div className="flex flex-wrap gap-2 sm:gap-3 overflow-y-auto max-h-28 sm:max-h-32 p-1">
                  {speakerStats?.topTags?.map((tag, index) => {
                    ...
                    return (
                      ...
                      >
                        {tag?.tag || "Unknown"}{" "}
                        <span className="inline-flex items-center justify-center bg-gray-100 text-gray-700 rounded-full h-4 w-4 sm:h-5 sm:w-5 ml-1 text-xs">
                          {tag?.count || 0}
                        </span>
                      </span>
                    );
                      ...
                      >
                        <div className="block">
                          <Link href={`/events/${talk.id}`}>
                            <h4 className="font-bold text-lg hover:text-yellow-600 transition-colors">
                              {talk.title}
                            </h4>
                          </Link>
                          <div className="flex flex-col sm:flex-row sm:items-center mt-2 text-sm text-gray-500">
                            <div className="flex items-center mb-2 sm:mb-0">
                              ...
                              <Link
                                ...
                                className="mr-4 hover:text-yellow-600 truncate"
                              >
                                {talk.speakerName}
                              </Link>
                            </div>
      ...
      <Section variant="black">
        ...
        >
          <h2 className="text-2xl sm:text-3xl font-bold mb-3 sm:mb-4">
            Ready to Share Your JavaScript Knowledge? 💡
          </h2>
          <p className="text-base sm:text-xl mb-6 sm:mb-8 max-w-2xl mx-auto">
            Join our lineup of amazing speakers! Submit your talk proposal and inspire the ZurichJS
            community.
          </p>
          <Button
            href="/cfp"
            variant="primary"
            size="lg"
            className="bg-yellow-500 text-black hover:bg-yellow-600 text-sm sm:text-base"
          >
...
export async function getStaticProps() {
  ...
  const totalTalks = speakers.reduce(
    (sum: number, speaker: Speaker) => sum + speaker.talks.length,
    0,
  );
  ...
  const totalTalkMinutes = speakers.reduce((sum: number, speaker: Speaker) => {
    return (
      sum +
      speaker.talks.reduce(
        (talkSum: number, talk: Talk) => talkSum + (talk.durationMinutes ?? 0),
        0,
      )
    );
  }, 0);
  ...
  const upcomingTalks: UpcomingTalk[] = talks
      ...
      talk.events!.map(
        (event) =>
          ({
            ...
            title: talk.title,
            speakerName:
              talk.speakers && talk.speakers.length > 0 ? talk.speakers[0].name : "Unknown Speaker",
    ...
    .sort(
      (a: TalkWithEventDate, b: TalkWithEventDate) => a.eventDate.getTime() - b.eventDate.getTime(),
    )
    .slice(0, 3)
