...
export default function AIFunMode({
  isOpen,
  onClose,
  imageUrl,
  imageAlt,
  ...
  const updateEffect = (effectId: string, value: string | number) => {
    setEffects((prev) =>
      prev.map((effect) => (effect.id === effectId ? { ...effect, value } : effect)),
    );
  };
            ...
            <div className="p-6">
                ...
                {effects.map((effect) => (
                  <div key={effect.id} className="bg-white/5 rounded-lg p-4">
                    <div className="flex items-center gap-2 mb-3">
                      <div className="p-1 bg-white/10 rounded">{effect.icon}</div>
                    ...
                    {effect.type === "toggle" && (
                      <div className="space-y-2">
                        <button
                          onClick={() =>
                            updateEffect(effect.id, effect.value === "true" ? "" : "true")
                          }
              ...
              <div className="mt-6 p-3 bg-zurich/20 border border-zurich/30 rounded-lg">
                <p className="text-xs text-zurich/80">
                  💡 Press <kbd className="px-1 bg-white/20 rounded text-xs">H</kbd> to toggle
                  controls
                </p>
              </div>
            </div>
