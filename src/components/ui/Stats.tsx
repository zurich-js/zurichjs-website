...
export default function Stats({
  ...
  return (
    <div
      className={`${backgroundColor} text-white rounded-xl shadow-xl p-6 backdrop-blur-sm max-w-screen-lg mx-auto`}
    >
      <motion.div
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
        {statItems.map((item, index) => (
          ...
          >
            ...
            {item.description && (
              <div className="text-xs text-gray-300 mt-1 italic">{item.description}</div>
            )}
          </motion.div>
        ))}
      </div>
