...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    if (userId !== refereeId) {
      ...
      return res
        .status(403)
        .json({ message: "Forbidden: User can only update their own referral data" });
    }
    ...
    try {
      ...
      if (
        currentMetadata?.referredBy &&
        (currentMetadata.referredBy as ReferredBy).userId === refereeId
      ) {
