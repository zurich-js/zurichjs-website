...
async function handleLegacyFeedback(
  feedback: LegacyFeedbackData,
  req: NextApiRequest,
  ...
  const talk = await sanityClient.fetch(`*[_type == "talk" && id.current == $talkId][0]`, {
    talkId,
  });
...
async function handleComprehensiveFeedback(
  feedback: ComprehensiveFeedbackData,
  req: NextApiRequest,
  res: NextApiResponse<ResponseData>,
) {
  const {
  ...
  try {
    ...
    const avgRating =
      Math.round(
        ((foodOptions.rating + drinks.rating + talks.rating + timing.rating + execution.rating) /
          5) *
          10,
      ) / 10;
