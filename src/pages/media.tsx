...
function createEventMedias(
  pastEvents: Event[],
  filesByFolder: Record<string, ImageKitFile[]>,
): EventMedia[] {
  return Object.entries(filesByFolder)
    .map(([folderId, files]) => {
  ...
  latestTwoEvents.forEach((eventMedia, eventIndex) => {
    ...
    if (photos.length > 0) {
      ...
      if (photos.length <= maxPhotosFromEvent) {
        ...
        while (
          selectedIndices.length < maxPhotosFromEvent &&
...
function getLatestVideo(eventMedias: EventMedia[]): { media: MediaItem; event: EventMedia } | null {
  const sortedEvents = [...eventMedias].sort(
    (a, b) =>
      new Date(b.event?.datetime || 0).getTime() - new Date(a.event?.datetime || 0).getTime(),
  );
...
export default function Media({ pastEvents }: MediaProps) {
  ...
  const [selectedImageForAI, setSelectedImageForAI] = useState<{ url: string; alt: string } | null>(
    null,
  );
      ...
      <Section variant="gradient" padding="lg">
        ...
        >
          <div className="text-center text-gray-900">
            <h1 className="text-4xl md:text-6xl font-bold mb-4 drop-shadow-lg">Media Gallery</h1>
            ...
            <p className="text-base md:text-lg opacity-80 max-w-3xl mx-auto">
              Dive into our rich collection of photos and videos from meetups, workshops, and
              special events. Each image tells a story of learning, networking, and community
              building.
            </p>
          ...
          <div className="flex flex-wrap justify-center gap-4 sm:gap-6 md:gap-8 mt-4">
            <div className="flex items-center bg-white/20 backdrop-blur-sm rounded-full px-4 sm:px-6 py-2 sm:py-3">
              ...
              <span className="font-medium text-sm sm:text-base text-gray-900">
                {allMedia.filter((item) => item.type === "photo").length} Photos
              </span>
            </div>
            <div className="flex items-center bg-white/20 backdrop-blur-sm rounded-full px-4 sm:px-6 py-2 sm:py-3">
              <Video className="w-4 h-4 sm:w-5 sm:h-5 mr-2 text-gray-900" />
              <span className="font-medium text-sm sm:text-base text-gray-900">
                {allMedia.filter((item) => item.type === "video").length} Videos
              </span>
            </div>
            <div className="flex items-center bg-white/20 backdrop-blur-sm rounded-full px-4 sm:px-6 py-2 sm:py-3">
              <Calendar className="w-4 h-4 sm:w-5 sm:h-5 mr-2 text-gray-900" />
              <span className="font-medium text-sm sm:text-base text-gray-900">
                {eventMedias.length} Events
              </span>
            </div>
            {aiFunModeActive && (
              <motion.div
              ...
              >
                ...
                <span className="font-medium text-sm sm:text-base text-blue-800 mr-3">
                  AI Fun Mode Active
                </span>
                <button
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 min-h-[400px]">
                <div className="relative lg:col-span-1">
                  <div className="h-full min-h-[400px] flex items-center justify-center">
                    <div
                      className="relative w-full bg-gradient-to-br from-gray-200 via-gray-300 to-gray-200 overflow-hidden rounded-2xl sm:rounded-3xl"
                ...
                <div className="lg:col-span-2 p-6 sm:p-8 lg:p-10 flex flex-col justify-center">
                  ...
                  <div className="flex items-center text-gray-700 text-sm sm:text-base mb-4">
                    ...
                    <span className="font-medium">
                      {latestVideo.event.event?.datetime
                        ? formatDate(latestVideo.event.event.datetime)
                        : "Date not available"}
                    </span>
                  </div>
                  ...
                  <p className="text-sm sm:text-base lg:text-lg text-gray-700 mb-6 sm:mb-8 line-clamp-3 leading-relaxed">
                    {latestVideo.event.event?.description ||
                      "Watch highlights from this amazing community event and see what you missed!"}
                  </p>
                  ...
                  <div className="flex flex-col sm:flex-row gap-3 sm:gap-4">
                    ...
                    <Button
                      onClick={() => {
                        const photos = latestVideo.event.media.filter(
          ...
          <div className="flex items-center justify-between mb-4 sm:mb-6">
            <div className="flex items-center gap-3">
              ...
              <h2 className="text-2xl sm:text-3xl font-bold text-gray-900">
                Featured Photos from Our Latest Events
              </h2>
            </div>
                  ...
                  <motion.div
                    ...
                    onClick={() => {
                      ...
                      if (eventMedia) {
                        const photos = eventMedia.media.filter((item) => item.type === "photo");
                        if (photos.length > 0) {
                          const clickedPhotoIndex = photos.findIndex(
      ...
      <Section variant="white" padding="lg">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-8">
            <h2 className="text-2xl sm:text-3xl font-bold text-gray-900">Event Gallery</h2>
          </div>
