      ...
      {steps.map((stepItem, index) => {
        ...
        return (
          <div key={stepItem.label} className="contents">
            ...
            >
              <div
                className={`relative w-10 h-10 sm:w-12 sm:h-12 rounded-full flex items-center justify-center transition-all duration-500 ${isActive ? "bg-black text-js shadow-lg" : isCompleted ? "bg-green-500 text-white" : "bg-gray-100"}`}
              >
                {isCompleted ? (
              ...
              <span className="text-xs sm:text-sm mt-1 sm:mt-2 font-semibold text-center">
                {stepItem.label}
              </span>
            </div>
            ...
            {index < steps.length - 1 && (
              <div
                className={`relative h-1 w-4 sm:w-6 md:w-8 rounded-full transition-all duration-700 flex-1 max-w-[2rem] ${step > index ? "bg-gradient-to-r from-black to-js" : "bg-gray-200"}`}
              >
                {step > index && (
