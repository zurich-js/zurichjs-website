...
const ProductFeedback = ({
  ...
  const updateFeedbackData = useCallback(
    (
      ...
      if (updates.learningPreferences !== undefined)
        setLearningPreferences(updates.learningPreferences);
      ...
      onChange({
        ...
        learningPreferences:
          updates.learningPreferences !== undefined
            ? updates.learningPreferences
            : learningPreferences,
        detailedFeedback:
          updates.detailedFeedback !== undefined ? updates.detailedFeedback : detailedFeedback,
      });
    },
  ...
  useEffect(() => {
    if (rating > 0) {
      ...
      if (
        (isHighRating && detailedFeedback.includes("could be improved")) ||
...
const FeedbackForm = ({
  ...
  const initialProductDemos = productDemos?.length
    ? productDemos
    : productDemo
      ? [productDemo]
      : [];
  const initialActiveDemo =
    productDemo || (initialProductDemos.length > 0 ? initialProductDemos[0] : null);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await onSubmit({
        ...
        productFeedback:
          productFeedback && activeProductDemo
            ? {
    ...
    return (
      ...
      >
        ...
        <h4 className="font-bold text-lg mb-2">
          Thank you for your feedback on &ldquo;{talkTitle}&rdquo;!
        </h4>
        <p className="text-gray-600 mb-3">
          Your insights help {speakerName} improve and our community grow.
        </p>
        ...
        {activeProductDemo && productFeedback && productFeedback.rating > 0 && (
          <div className="mt-3 pt-3 border-t border-green-200">
            <p className="text-gray-600">
              Your product feedback for <strong>{activeProductDemo.name}</strong> has been recorded.
            </p>
            <p className="text-sm text-green-600">
              Thank you for helping our sponsors improve their products!
            </p>
          </div>
        )}
        ...
        {allProductDemos.length > 1 && !productFeedback && (
          <div className="mt-3 pt-3 border-t border-green-200">
            ...
            <p className="text-sm text-green-600">
              Consider providing feedback for other products as well!
            </p>
          </div>
        )}
      </motion.div>
  ...
  const commentPresets = [
    ...
    {
      label: "Learned a lot",
      value: "I learned a lot from this talk and will apply it in my work.",
    },
    {
      label: "Engaging speaker",
      value: "The speaker was engaging and kept my attention throughout.",
    },
    {
      label: "Technical issues",
      value: "There were some technical issues that affected the presentation.",
    },
    {
      label: "Would like more depth",
      value: "The topic was interesting but I would have liked more in-depth content.",
    },
  ...
  const applyPreset = (preset: string) => {
    ...
    const existingPresetIndex = commentPresets.findIndex((p) => comment.includes(p.value));
      ...
      {(!isLoaded || !user) && (
        <div className="space-y-2">
          ...
          <p className="text-xs text-gray-500">
            Share your email if you&apos;d like us to follow up about your feedback or product demo
            interest
          </p>
          <input
            id={`email-${talkId}`}
    ...
    const mockDemos = [
      {
        ...
        name: "SonarQube",
        description:
          "Static code analysis tool that helps developers write cleaner and safer code.",
    ...
    >
      ...
      {completionPercentage === 100 && (
        ...
        >
          ...
          <p className="text-gray-700">
            You&apos;ve completed feedback for all talks. Your input is incredibly valuable to our
            community!
          </p>
        </motion.div>
      )}
