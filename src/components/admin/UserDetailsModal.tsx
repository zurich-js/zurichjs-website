...
export default function UserDetailsModal({ user, isOpen, onClose }: UserDetailsModalProps) {
  const [activeTab, setActiveTab] = useState<"overview" | "survey" | "coupons" | "credits">(
    "overview",
  );
  ...
  const handleToggleCoupon = async (couponCode: string, isActive: boolean) => {
    ...
    setOptimisticCoupons((prev) =>
      prev.map((coupon) => (coupon.code === couponCode ? { ...coupon, isActive } : coupon)),
    );
    ...
    try {
      ...
      if (!response.ok) {
        ...
        setOptimisticCoupons((prev) =>
          prev.map((coupon) =>
            coupon.code === couponCode ? { ...coupon, isActive: !isActive } : coupon,
          ),
        );
                    ...
                    <div className="bg-gray-50 p-4 rounded-lg">
                      ...
                      <div className="space-y-3">
                        ...
                        <div>
                          ...
                          <p className="text-sm">{user.unsafeMetadata!.surveyData!.company}</p>
                        </div>

                        <div>
                          <p className="text-xs text-gray-500">Experience Level</p>
                          <p className="text-sm">{user.unsafeMetadata!.surveyData!.experience}</p>
                        </div>
                      </div>
                    </div>
                  ...
                  <div className="mt-6 bg-gray-50 p-4 rounded-lg">
                    ...
                    <div className="space-y-3">
                      <div>
                        ...
                        <p className="text-sm">
                          {user.unsafeMetadata!.surveyData!.newsletter
                            ? "Subscribed"
                            : "Not subscribed"}
                        </p>
                      </div>
                ...
                {user?.unsafeMetadata?.referredBy && (
                  <div className="mt-6 p-4 bg-blue-50 rounded-lg">
                    <p className="text-sm font-medium text-blue-800">
                      ...
                      <span className="font-bold">{user.unsafeMetadata.referredBy.name}</span> on{" "}
                      {new Date(user.unsafeMetadata.referredBy.date).toLocaleDateString()}
                    </p>
                  </div>
                )}
              </div>
